namespace NServiceBus.SagaPersisters.InMemory.Tests
{
    ...
    class When_multiple_workers_retrieve_same_saga
    {
        ...
        public async Task Save_fails_when_data_changes_between_read_and_update()
        {
            ...
            await persister.Save(returnedSaga1, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag());
            var exception = Assert.Throws<Exception>(() => persister.Save(returnedSaga2, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag()));
        ...
        public async Task Save_fails_when_data_changes_between_read_and_update_on_same_thread()
        {
            ...
            await persister.Save(record, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag());
            var exception = Assert.Throws<Exception>(() => persister.Save(staleRecord, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag()));
            Assert.IsTrue(exception.Message.StartsWith($"InMemorySagaPersister concurrency violation: saga entity Id[{saga.Id}] already saved."));
        }
        ...
        public async Task Save_fails_when_writing_same_data_twice()
        {
            ...
            var exception = Assert.Throws<Exception>(() => persister.Save(returnedSaga1, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag()));
        ...
        public async Task Save_process_is_repeatable()
        {
            ...
            await persister.Save(returnedSaga1, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag());
            var exceptionFromSaga2 = Assert.Throws<Exception>(() => persister.Save(returnedSaga2, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag()));
            ...
            await persister.Save(returnedSaga4, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag());

            var exceptionFromSaga3 = Assert.Throws<Exception>(() => persister.Save(returnedSaga3, SagaMetadataHelper.GetMetadata<TestSaga>(), new ContextBag()));

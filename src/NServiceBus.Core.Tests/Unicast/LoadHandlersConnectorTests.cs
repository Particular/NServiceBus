...
using Core.Tests.OpenTelemetry;
...
public class LoadHandlersConnectorTests
{
    ...
    {
        var behavior = new LoadHandlersConnector(new MessageHandlerRegistry(), NoOpActivityFactory.Instance, new IncomingPipelineMetrics(new TestMeterFactory(), "queue", "disc", new MetersOptions()));
    ...
    public void Should_not_throw_if_ambient_scope_is_same_as_transport_scope()
    {
        ...
        var behavior = new LoadHandlersConnector(messageHandlerRegistry, NoOpActivityFactory.Instance, new IncomingPipelineMetrics(new TestMeterFactory(), "queue", "disc", new MetersOptions()));

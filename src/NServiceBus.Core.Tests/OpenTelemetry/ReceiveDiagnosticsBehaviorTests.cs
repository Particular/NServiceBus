...
class ReceiveDiagnosticsBehaviorTests
{
    ...
    public async Task Should_increase_total_fetched_when_processing_message()
    {
        ...
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalFetched.Name, 1);

        var fetchedTags = metricsListener.Tags[MessagingMetricsMeters.TotalFetched.Name].ToImmutableDictionary();
    ...
    public async Task Should_increase_total_successful_when_processing_message_successfully()
    {
        ...
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalFetched.Name, 1);
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalProcessedSuccessfully.Name, 1);
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalFailures.Name, 0);

        var processedTags = metricsListener.Tags[MessagingMetricsMeters.TotalProcessedSuccessfully.Name].ToImmutableDictionary();
    ...
    public void Should_increase_failures_error_when_processing_message_fails()
    {
        ...
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalFetched.Name, 1);
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalProcessedSuccessfully.Name, 0);
        metricsListener.AssertMetric(MessagingMetricsMeters.TotalFailures.Name, 1);

        var failureTags = metricsListener.Tags[MessagingMetricsMeters.TotalFailures.Name].ToImmutableDictionary();

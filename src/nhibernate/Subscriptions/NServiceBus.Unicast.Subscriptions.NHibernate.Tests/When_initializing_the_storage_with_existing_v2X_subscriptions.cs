    ...
    public class When_initializing_the_storage_with_existing_v2X_subscriptions : InMemoryDBFixture
    {
        ...
        public void Should_automatically_update_them_to_the_30_format()
        {
            using (var session = subscriptionStorageSessionProvider.OpenSession())
            {
                ...
                cmd.CommandText = string.Format("INSERT INTO Subscription([SubscriberEndpoint],[MessageType]) values ('{0}','{1}')", TestClients.ClientA.FullName, typeof(MessageB).AssemblyQualifiedName);
            ...
            Assert.AreEqual(subscriptionsForMessageType.First().FullName,TestClients.ClientA.FullName);
        }
    }
}
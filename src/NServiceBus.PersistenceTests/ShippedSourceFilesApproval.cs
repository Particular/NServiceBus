// This test project is shipped as a source package (NServiceBus.PersistenceTests.Sources) to
// downstream persistence implementers. By default, Particular.Packaging includes ALL .cs files
...
// - If the file should NOT be shipped (e.g. it tests learning-persistence internals),
...
public class ShippedSourceFilesApproval
{
    ...
    public void ApproveShippedSourceFiles()
    {
        ...
        while (!File.Exists(Path.Combine(projectDirectory, "NServiceBus.PersistenceTests.csproj")))
        {
            projectDirectory = Directory.GetParent(projectDirectory)?.FullName
                ?? throw new InvalidOperationException("Could not find project directory.");
        }

        var csprojPath = Path.Combine(projectDirectory, "NServiceBus.PersistenceTests.csproj");

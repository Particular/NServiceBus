namespace NServiceBus.AcceptanceTests.Core.Hosting
{
    ...
    public class When_disabling_assembly_scanning
    {
        ...
        public void Should_not_start_endpoint()
        {
            ...
            StringAssert.Contains($"Assembly scanning has been disabled. This prevents messages, message handlers, features and other functionality to not load correctly. Enable either {nameof(AssemblyScannerConfiguration.ScanAppDomainAssemblies)} and/or {nameof(AssemblyScannerConfiguration)}", exception.Message);
        }

...
public class ShippedSourceFilesApproval
{
    ...
    public void ApproveShippedSourceFiles()
    {
        ...
        while (!File.Exists(Path.Combine(projectDirectory, "NServiceBus.AcceptanceTests.csproj")))
        {
            projectDirectory = Directory.GetParent(projectDirectory)?.FullName
                ?? throw new InvalidOperationException("Could not find project directory.");
        }

        var csprojPath = Path.Combine(projectDirectory, "NServiceBus.AcceptanceTests.csproj");

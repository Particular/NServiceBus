    ...
    public class When_deferring_outer_send : NServiceBusAcceptanceTest
    {
        ...
        class SenderWithNestedSend : EndpointConfigurationBuilder
        {
            public SenderWithNestedSend() => EndpointSetup<DefaultServer>((c, r) =>
            {
                ...
                {
                    c.Pipeline.Register(new DeliveryConstraintObserver((Context)r.ScenarioContext), "Tracks whether a message being dispatched is marked for delayed delivery");
                }
            });
            ...
            class DeliveryConstraintObserver : Behavior<IDispatchContext>
            {
                Context testContext;

                public DeliveryConstraintObserver(Context testContext)
                {
                ...
                {
                    if (context.Extensions.TryGetDeliveryConstraint(out DelayDeliveryWith _))
                    {
                        var outgoingMessage = context.Extensions.Get<OutgoingLogicalMessage>();
                        if (outgoingMessage.MessageType == typeof(DelayedMessage))
                        {
                            testContext.DelayedMessageDelayed = true;
                        }

                        if (outgoingMessage.MessageType == typeof(NonDelayedMessage))
                        {
                            testContext.NonDelayedMessageDelayed = true;
                        }
                    }

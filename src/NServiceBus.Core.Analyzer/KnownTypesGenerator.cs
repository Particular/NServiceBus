...
public sealed class KnownTypesGenerator : IIncrementalGenerator
{
    ...
    static IEnumerable<ScannedTypeInfo> FindMatchingMarkers(INamedTypeSymbol? type, ImmutableArray<MarkerTypeInfo> markers, CancellationToken cancellationToken)
    {
        ...
        foreach (var marker in markers)
        {
            ...
            {
                var namespaceStr = type.ContainingNamespace?.ToDisplayString() ?? string.Empty;
                yield return new ScannedTypeInfo(type.ToDisplayString(), marker.RegisterMethod, marker.AutoRegister, namespaceStr);
            }
        }
    }
    ...
    static void GenerateRegistrationCode(SourceProductionContext sourceProductionContext, ImmutableArray<ScannedTypeInfo> matches)
    {
        ...
        // Detect endpoint partitions by namespace patterns
        var endpointPartitions = DetectEndpointPartitions(matches);
        ...
        // Generate global OptionalTypeRegistration (all types, for backward compatibility)
        sb.AppendLine("""
        ...
        // Generate partitioned registration classes for each endpoint
        foreach (var kvp in endpointPartitions.OrderBy(kvp => kvp.Key))
        {
            var endpointName = kvp.Key;
            var types = kvp.Value;

            sb.AppendLine($"        [NServiceBus.Extensibility.AutoGeneratedTypeRegistrationsAttribute(false)]");
            sb.AppendLine($"        public static class {endpointName}Registration");
            sb.AppendLine("        {");
            sb.AppendLine("            public static void RegisterTypes(NServiceBus.EndpointConfiguration config)");
            sb.AppendLine("            {");

            foreach (var type in types.Where(t => !t.AutoRegister).OrderBy(t => t.DisplayName))
            {
                sb.AppendLine($"                config.{type.RegisterMethod}<{type.DisplayName}>();");
            }
        ...
        // Generate extension methods for the partitioned registrations
        GenerateExtensionMethods(sourceProductionContext, endpointPartitions);
    }

    static void GenerateExtensionMethods(SourceProductionContext sourceProductionContext, Dictionary<string, List<ScannedTypeInfo>> endpointPartitions)
    {
        var sb = new StringBuilder();
        sb.AppendLine("""
                      // <auto-generated/>
                      ...
                      {
                          public static class GeneratedRegistrationExtensions
                          {
                              /// <summary>
                              /// Registers all types discovered by the source generator (single endpoint scenario).
                              /// </summary>
                              public static void UseGeneratedRegistrations(this NServiceBus.EndpointConfiguration endpointConfiguration)
                              {
                                  NServiceBus.Generated.RequiredTypeRegistration.RegisterTypes(endpointConfiguration);
                                  NServiceBus.Generated.OptionalTypeRegistration.RegisterTypes(endpointConfiguration);
                              }

                      """);

        // Generate endpoint-specific methods
        foreach (var kvp in endpointPartitions.OrderBy(kvp => kvp.Key))
        {
            var endpointName = kvp.Key;

            sb.AppendLine("            /// <summary>");
            sb.AppendLine($"            /// Registers types for the {endpointName} endpoint (multi-endpoint scenario).");
            sb.AppendLine("            /// </summary>");
            sb.AppendLine($"            public static void UseGeneratedRegistrations_{endpointName}(this NServiceBus.EndpointConfiguration endpointConfiguration)");
            sb.AppendLine("            {");
            sb.AppendLine("                NServiceBus.Generated.RequiredTypeRegistration.RegisterTypes(endpointConfiguration);");
            sb.AppendLine($"                NServiceBus.Generated.{endpointName}Registration.RegisterTypes(endpointConfiguration);");
            sb.AppendLine("            }");
            sb.AppendLine();
        }
        ...
        sourceProductionContext.AddSource("GeneratedRegistrationExtensions.g.cs", sb.ToString());
    }

    static Dictionary<string, List<ScannedTypeInfo>> DetectEndpointPartitions(ImmutableArray<ScannedTypeInfo> matches)
    {
        var partitions = new Dictionary<string, List<ScannedTypeInfo>>();

        foreach (var type in matches.Where(t => !t.AutoRegister)) // Only partition optional types
        {
            var endpointName = ExtractEndpointName(type.Namespace);
            if (!string.IsNullOrEmpty(endpointName))
            {
                if (!partitions.TryGetValue(endpointName, out var list))
                {
                    list = [];
                    partitions[endpointName] = list;
                }
    ...
    static string ExtractEndpointName(string namespaceName)
    {
        ...
        // Look for patterns like: MyApp.Sales.*, MyApp.Billing.*, etc.
        // Extract the endpoint name (second segment after root)
        var parts = namespaceName.Split('.');
        if (parts.Length >= 2)
        {
            // Return the second segment as the endpoint name
            return parts[1];
        }
    ...
    record struct ScannedTypeInfo(string DisplayName, string RegisterMethod, bool AutoRegister, string Namespace);

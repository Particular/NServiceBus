...
public sealed class KnownTypesGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        {
            // Provide the NServiceBus.NsbHandler attribute so consumers can use it without extra refs
            ctx.AddSource("NsbAttributes.g.cs", SourceText.From(
                """
                #nullable enable
                // <auto-generated/>
                namespace NServiceBus
                {
                    ...
                    public sealed class NsbHandlerAttribute : Attribute
                    {
                       
                    }

                    [AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
                    public sealed class NsbSagaAttribute : Attribute
                    {
                       
                    }

                    [AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
                    public sealed class NsbFeatureAttribute : Attribute
                    {
        ...
        var nsbHandlerTypes = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                "NServiceBus.NsbHandlerAttribute",
                predicate: static (node, _) => node is TypeDeclarationSyntax,
                transform: GetNamedTypeFromAttributeContext)
            .Where(type => type is not null)
            .Select((type, cancellationToken) => new ScannedTypeInfo(type!.ToDisplayString()));

        // Check for types marked with [NsbSaga] attribute
        var nsbSagaTypes = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                "NServiceBus.NsbSagaAttribute",
                predicate: static (node, _) => node is TypeDeclarationSyntax,
                transform: GetNamedTypeFromAttributeContext)
            .Where(type => type is not null)
            .Select((type, cancellationToken) => new ScannedTypeInfo(type!.ToDisplayString()));

        // Check for types marked with [NsbFeature] attribute
        var nsbFeatureTypes = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                "NServiceBus.NsbFeatureAttribute",
                predicate: static (node, _) => node is TypeDeclarationSyntax,
                transform: GetNamedTypeFromAttributeContext)
            .Where(type => type is not null)
            .Select((type, cancellationToken) => new ScannedTypeInfo(type!.ToDisplayString()));
        ...
        var collectedNsbHandler = nsbHandlerTypes.Collect();
        var collectedNsbSaga = nsbSagaTypes.Collect();
        var collectedNsbFeature = nsbFeatureTypes.Collect();

        var combined = collectedRelevant
            .Combine(collectedNsbHandler)
            .Combine(collectedNsbSaga)
            .Combine(collectedNsbFeature)

...
public sealed class KnownTypesGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(ctx =>
        {
            ...
            ctx.AddSource("NsbHandlerAttribute.g.cs", SourceText.From(
                """
                #nullable enable
                // <auto-generated/>
        ...
        // Collect both interface-based and attribute-based types separately, then combine
        var collectedRelevant = relevantTypes.Collect();
        var collectedNsbHandler = nsbHandlerTypes.Collect();

        var combined = collectedRelevant.Combine(collectedNsbHandler)
            .Select((tuple, cancellationToken) =>
            {
                var (relevant, nsbHandler) = tuple;
                return relevant.Concat(nsbHandler).ToImmutableArray();
            });

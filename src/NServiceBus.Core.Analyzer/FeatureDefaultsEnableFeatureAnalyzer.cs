...
public class FeatureDefaultsEnableFeatureAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        {
            if (!FeatureKnownTypes.TryGet(startContext.Compilation, out var knownTypes))
            {
                return;
            }

            startContext.RegisterSyntaxNodeAction(
                ctx => AnalyzeEnableInvocation(ctx, knownTypes.Feature, knownTypes.SettingsExtensions),

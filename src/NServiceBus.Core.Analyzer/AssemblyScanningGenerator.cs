...
public sealed class AssemblyScanningGenerator : IIncrementalGenerator
{
    ...
    record struct InterceptorsAndRegistrationClasses(ImmutableArray<AutoGeneratedMethod> Methods, ImmutableArray<InvocationDetails?> Locations);
    ...
    static InvocationDetails? TransformToInterceptor(GeneratorSyntaxContext context, CancellationToken cancellationToken)
    {
        ...
        return new InvocationDetails(operation.TargetMethod.Name, location);
    }

    record struct InvocationDetails(string MethodName, InterceptableLocation Location);
    ...
    static void GenerateInterceptorCode(SourceProductionContext sourceProductionContext, InterceptorsAndRegistrationClasses data)
    {
        ...
        var locationsByMethodName = locations.ToLookup(location => location!.Value.MethodName);
        ...
        var useSourceGenMethods = locationsByMethodName[UseSourceGenMethodName].OfType<InvocationDetails>().ToImmutableArray();
        var registerHandlersMethods = locationsByMethodName[RegisterHandlersMethodName].OfType<InvocationDetails>().ToImmutableArray();
        ...
        if (useSourceGenMethods.Any())
        {
            ...
            foreach (var method in methods.Where(m => m.AutoRegister))
            {
        ...
        if (registerHandlersMethods.Any())
        {
            ...
            foreach (var method in methods.Where(m => !m.AutoRegister))
            {
                sb.AppendLine($"            {method.ClassName}.RegisterTypes(options.Configuration);");
            }

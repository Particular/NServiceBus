...
public abstract class RegisterStep
{
    ...
    protected RegisterStep(string stepId, Type behavior, string? description, Func<IServiceProvider, IBehavior>? factoryMethod = null)
    {
        ...
        this.factoryMethod = factoryMethod ?? (provider => (IBehavior)ActivatorUtilities.CreateInstance(provider, BehaviorType));
    }
    ...
    internal void Replace(ReplaceStep replacement)
    {
        ...
        factoryMethod = replacement.FactoryMethod ?? (provider => (IBehavior)ActivatorUtilities.CreateInstance(provider, BehaviorType));
